#!/usr/bin/env bash
# Example: ROS 2 Humble colcon workspace (taken from the RX150 project).
#
...
STEPS=(
  "build|1800|./tools/build.sh"
  "reach|600|./rx150.sh reach"
  "offline_tests|600|./rx150.sh test"
  "pytest|600|source ./source_all.sh >/dev/null && export PYTHONNOUSERSITE=1 && python3 -m pytest -q -p no:cacheprovider src/rx150/rx150_toolbox/rx150_modules/test src/rx150/apps/rx150_pick_place/test"
)
...
# (~/.bashrc of this machine sources another workspace's overlay; CLEAN_ENV=1 keeps it
# out, and steps that need ROS source the workspace themselves.)
SETUP=""
